...
function loadState() {
  ...
  return { tasks: {}, scheduled: {}, scheduledMessageIds: {}, processedMessageTs: {}, lastWeeklySummaryDate: null, lastRun: null };
}
...
async function extractTasks(messages, userMap, state) {
  const today = new Date().toISOString().split('T')[0];
  ...
  // Filter out messages we've already extracted tasks from
  const unprocessed = enriched.filter(line => {
    const tsMatch = line.match(/\[ts:([^\]]+)\]/);
    if (!tsMatch) return true;
    const ts = tsMatch[1];
    return !state.processedMessageTs[ts];
  });

  if (unprocessed.length === 0) {
    console.log('All messages already processed — skipping extraction.');
    return [];
  }
  console.log(`Processing ${unprocessed.length} new messages (${enriched.length - unprocessed.length} already seen).`);

  const CHUNK_SIZE = 15;
  const allTasks = [];

  for (let i = 0; i < unprocessed.length; i += CHUNK_SIZE) {
    const chunk = unprocessed.slice(i, i + CHUNK_SIZE);
    const chunkNum = Math.floor(i / CHUNK_SIZE) + 1;
    const totalChunks = Math.ceil(unprocessed.length / CHUNK_SIZE);
  ...
  const deduplicatedByTs = allTasks.filter(t => {
  ...
  // Further deduplicate within this batch using fuzzy matching
  // (catches same task extracted with different wording in same scan)
  const deduplicated = [];
  for (const task of deduplicatedByTs) {
    const normNew = normalizeTask(task.task);
    const newAssignee = (task.assignee_name || '').toLowerCase().trim();
    let isDupe = false;
    for (const existing of deduplicated) {
      ...
      if (!existAssignee.includes(newAssignee.split(' ')[0]) &&
          !newAssignee.includes(existAssignee.split(' ')[0])) continue;
      const normExist = normalizeTask(existing.task);
      if (similarity(normNew, normExist) >= 0.72) {
        isDupe = true;
        break;
      }
    }
    if (!isDupe) deduplicated.push(task);
  }

  // Mark all processed message timestamps so we never re-extract them
  for (const line of unprocessed) {
    const tsMatch = line.match(/\[ts:([^\]]+)\]/);
    if (tsMatch) state.processedMessageTs[tsMatch[1]] = true;
  }
  ...
  for (const reminder of reminders) {
    ...
    try {
      const result = await slackPost('chat.scheduleMessage', {
        channel: SLACK_MANAGER_ID, text: msgs.manager, post_at: postAt, unfurl_links: false, unfurl_media: false
      });
    ...
    if (assigneeId && assigneeId !== SLACK_MANAGER_ID) {
      try {
        const result = await slackPost('chat.scheduleMessage', {
          channel: assigneeId, text: msgs.assignee, post_at: postAt, unfurl_links: false, unfurl_media: false
        });
        if (result.scheduled_message_id) {
          state.scheduledMessageIds[taskId].push({
            channel: assigneeId,
    ...
    if (fdChannelId) {
      try {
        const result = await slackPost('chat.scheduleMessage', {
          channel: fdChannelId, text: msgs.channel, post_at: postAt, unfurl_links: false, unfurl_media: false
        });
        if (result.scheduled_message_id) {
          state.scheduledMessageIds[taskId].push({
            channel: fdChannelId,
...
async function main() {
  ...
  try {
    ...
    const tasks = await extractTasks(realMessages, userMap, state);
    if (tasks.length > 0) {
      let newCount = 0;
      let mergedCount = 0;
    ...
    // Fires if: FORCE_WEEKLY_SUMMARY=true (triggered by cron-job.org dedicated job)
    // OR: it's Friday 9 AM EST and hasn't run yet this week (fallback)
    const thisWeek = getThisWeekStr();
    const forceSummary = process.env.FORCE_WEEKLY_SUMMARY === 'true';
    const autoSummary = isFriday() && new Date().getUTCHours() === REMINDER_HOUR_UTC
      && state.lastWeeklySummaryDate !== thisWeek;

    if (forceSummary || autoSummary) {
      console.log(forceSummary
        ? '\nForced weekly summary (triggered by cron-job.org)...'
