...
// MAX_OVERDUE_DAYS removed â€” overdue now schedules one reminder at a time
...
// â”€â”€â”€ Team channel mapping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const TEAM_CHANNEL_MAP = {
  'UMS39RAGP':   'C05HV4SQD6G',  // Brandon   â†’ #fd-brandon
  'U023F61R16F': 'C05JGF817H6',  // Mark      â†’ #fd-mark
  'U03KVJD9927': 'C08RPJDC073',  // Joma      â†’ #fd-joma
  'U01M7FMPTRR': 'C08RNTW55JM',  // Chrissy   â†’ #fd-to-do-chrissy
  'U07FVUF09T7': 'C093VPK55NF',  // Josh      â†’ #fd-josh
  'U02LFUG2WLR': 'C05HPPDCSS1',  // Eddie     â†’ #fd-eddie
  'U08D4RE1SKF': 'C097HTJ0Z5G',  // Liz       â†’ #fd-liz
  'U02773FLDN2': 'C097208ALLF',  // Jimmy     â†’ #fd-jimmy
  'UMCPG3D8A':   'C08R493ESAJ',  // Christle  â†’ #fd-christle
  'U05FE9QNFSQ': 'C05HL1VGA22',  // Megan     â†’ #fd-megan
};

const COMPLETION_KEYWORDS = [
  'done', 'completed', 'finished', 'complete', 'wrapped up', 'wrapped',
  'submitted', 'delivered', 'sent', 'âœ…', ':white_check_mark:'
];
...
// â”€â”€â”€ Slack helpers with retry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (res.status === 429) {
        ...
        console.warn(`  â³ Rate limited. Waiting ${wait/1000}s...`);
        await sleep(wait); attempt++; continue;
      }
      if (res.status >= 500) {
        const wait = Math.pow(2, attempt) * 1000;
        console.warn(`  â³ Server error ${res.status}. Waiting ${wait/1000}s...`);
        await sleep(wait); attempt++; continue;
      }
...
// â”€â”€â”€ Claude API with retry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries && (isOverloaded || isRateLimit || isServerErr)) {
        ...
        console.warn(`  â³ Anthropic API ${e.status || 'error'} â€” waiting ${wait/1000}s before retry ${attempt+1}/${maxRetries}...`);
...
// â”€â”€â”€ Fetch messages â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Build user map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Completion detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function detectCompletions(messages, userMap, existingTasks) {
  ...
  for (const [taskId, taskState] of openTasks) {
    ...
    if (hasCompletion) {
      console.log(`  âœ… Thread reply signals completion: "${taskState.task}"`);
...
If none: {"completed": []}
Only mark complete on clear signals: "done", "completed", "finished", "submitted", "âœ…".`,
...
// â”€â”€â”€ Extract tasks via Claude (with chunking for large message sets) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
Return ONLY a valid complete JSON object â€” no markdown, no explanation, no code fences, no truncation:
...
- Include ALL assignments â€” casual, formal, @mentioned, implied
...
async function extractTasks(messages, userMap) {
  ...
  // Chunk messages to avoid token limits â€” process in groups of 15
  const CHUNK_SIZE = 15;
  const allTasks = [];
...
// â”€â”€â”€ Reminder date logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function buildMessages(reminder, task, assigneeMention) {
  ...
  switch (reminder.type) {
    case 'midpoint':
      return {
        manager:  `*Midpoint check-in* ðŸ“‹\n*Task:* ${task.task}\n*Assigned to:* ${assigneeMention}\n*Due:* ${dueLabel}\n\nHalfway to the deadline â€” consider checking on progress with ${assigneeMention}.`,
        assignee: `Hey ${assigneeMention}! ðŸ‘‹ Midpoint check-in:\n\n*Task:* ${task.task}\n*Due:* ${dueLabel}\n\nHow's progress? ${managerMention} wanted a quick status update.`,
        channel:  `${assigneeMention} â€” midpoint check-in ðŸ“‹\n\n*Task:* ${task.task}\n*Due:* ${dueLabel}\n\nHalfway there! Loop in ${managerMention} with a status update.`
      };
    case 'nag': {
      const d = reminder.daysUntilDue;
      const urgency = d === 1 ? 'ðŸš¨ *1 day left!*' : `âš ï¸ *${d} days left*`;
      return {
        ...
        channel:  `${assigneeMention} â€” ${urgency}:\n\n*Task:* ${task.task}\n*Due:* ${dueLabel}\n\nUpdate ${managerMention} on your status!`
      };
    }
    case 'due':
      return {
        manager:  `*Task due today* ðŸ“‹\n*Task:* ${task.task}\n*Assigned to:* ${assigneeMention}\n*Due:* ${dueLabel}\n\nThis task is due today â€” check in with ${assigneeMention}.`,
        assignee: `Hey ${assigneeMention}! â° Task due *today*:\n\n*Task:* ${task.task}\n*Due:* ${dueLabel}\n\nGet it wrapped up! Reach out to ${managerMention} if you need anything.`,
        channel:  `${assigneeMention} â€” task due *today* â°\n\n*Task:* ${task.task}\n*Due:* ${dueLabel}\n\nDue today! Let ${managerMention} know if you need help.`
      };
    case 'overdue': {
      const d = reminder.daysOverdue;
      const dayLabel = d === 1 ? '1 day' : `${d} days`;
      return {
        manager:  `*Task overdue* ðŸš¨\n*Task:* ${task.task}\n*Assigned to:* ${assigneeMention}\n*Was due:* ${dueLabel}\n*Overdue by:* ${dayLabel}\n\nThis task has not been marked complete.`,
        assignee: `Hey ${assigneeMention}! ðŸš¨ This task is *overdue by ${dayLabel}*:\n\n*Task:* ${task.task}\n*Was due:* ${dueLabel}\n\nPlease complete this ASAP and notify ${managerMention}.`,
        channel:  `${assigneeMention} â€” *overdue by ${dayLabel}* ðŸš¨\n\n*Task:* ${task.task}\n*Was due:* ${dueLabel}\n\nThis is past due! Update ${managerMention} immediately.`
      };
    }
    default: // followup
      return {
        manager:  `*Weekly follow-up* ðŸ“‹\n*Task:* ${task.task}\n*Assigned to:* ${assigneeMention}\n*Assigned:* ${formatDate(task.assigned_date)}\n\nNo due date set â€” hasn't been marked complete after 7 days.`,
        assignee: `Hey ${assigneeMention}! ðŸ‘‹ Following up on an open task:\n\n*Task:* ${task.task}\n\nNo due date was set â€” what's the status? Loop in ${managerMention}.`,
        channel:  `${assigneeMention} â€” weekly follow-up ðŸ“‹\n\n*Task:* ${task.task}\n\nOpen for 7 days with no due date. What's the status? Let ${managerMention} know.`
      };
  }
}

// â”€â”€â”€ Schedule reminders for a task â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function scheduleRemindersForTask(task, taskId, userMap, state) {
  if (state.tasks[taskId]?.completed) {
    console.log(`  â†’ Completed â€” skipping: "${task.task}"`);
    return;
  }

  const reminders = calcReminders(task);
  if (reminders.length === 0) {
    console.log(`  â†’ No upcoming reminders for: "${task.task}"`);
    return;
  }
  ...
  for (const reminder of reminders) {
    ...
    if (state.scheduled[key]) {
      console.log(`  â†’ Already scheduled ${reminder.type} (${reminder.dateStr}) â€” skipping`);
      continue;
    }
    ...
    if (postAt <= nowSec + 120) {
      console.log(`  â†’ Past/too soon â€” skipping ${reminder.type} (${reminder.dateStr})`);
    ...
    try {
      ...
      console.log(`  âœ“ Manager DM â€” ${reminder.type} on ${reminder.dateStr}`);
      scheduled = true;
    } catch (e) { console.error(`  âœ— Manager DM failed: ${e.message}`); }
    ...
    if (assigneeId && assigneeId !== SLACK_MANAGER_ID) {
      try {
        ...
        console.log(`  âœ“ Assignee DM â€” ${task.assignee_name}`);
      } catch (e) { console.error(`  âœ— Assignee DM failed: ${e.message}`); }
    } else if (!assigneeId) {
      console.log(`  âš  Could not resolve Slack ID for "${task.assignee_name}"`);
    }

    if (fdChannelId) {
      try {
        await slackPost('chat.scheduleMessage', { channel: fdChannelId, text: msgs.channel, post_at: postAt });
        console.log(`  âœ“ #fd- channel â€” ${task.assignee_name}`);
      } catch (e) { console.error(`  âœ— #fd- channel failed: ${e.message}`); }
    }
...
// â”€â”€â”€ Weekly Friday summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function isFriday() {
  const now = new Date();
...
async function sendWeeklySummary(state) {
  ...
  const lines = [];
  lines.push(`*ðŸ“Š Weekly Task Summary â€” ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'America/New_York' })}*`);
  lines.push(`_${completed.length} completed Â· ${open.length} open Â· ${overdue.length} overdue_\n`);

  if (overdue.length > 0) {
    lines.push(`*ðŸš¨ OVERDUE (${overdue.length})*`);
    for (const t of overdue) {
      ...
      lines.push(`â˜  ${t.task}\n     â†’ ${assigneeMention} | Was due: ${formatDate(t.due_date)} | *${daysLate}d late*`);
    }
    lines.push('');
  }

  if (dueThisWeek.length > 0) {
    lines.push(`*âš ï¸ DUE THIS WEEK (${dueThisWeek.length})*`);
    for (const t of dueThisWeek) {
      const assigneeMention = t.assigneeId ? `<@${t.assigneeId}>` : t.assignee_name;
      lines.push(`â˜  ${t.task}\n     â†’ ${assigneeMention} | Due: ${formatDate(t.due_date)}`);
    }
    lines.push('');
  }

  if (onTrack.length > 0) {
    lines.push(`*ðŸ“Œ IN PROGRESS (${onTrack.length})*`);
    for (const t of onTrack) {
      const assigneeMention = t.assigneeId ? `<@${t.assigneeId}>` : t.assignee_name;
      const dueStr = t.due_date ? `Due: ${formatDate(t.due_date)}` : 'No due date';
      lines.push(`â˜  ${t.task}\n     â†’ ${assigneeMention} | ${dueStr}`);
    }
    lines.push('');
  }

  if (completed.length > 0) {
    lines.push(`*âœ… COMPLETED THIS PERIOD (${completed.length})*`);
  ...
  try {
    ...
    console.log('âœ“ Weekly summary posted to #ampd-team-tasks');
  } catch (e) {
    console.error(`âœ— Weekly summary failed: ${e.message}`);
  }
}
...
async function main() {
  ...
  try {
    ...
    if (completedIds.length > 0) {
      ...
      for (const id of completedIds) {
        if (state.tasks[id] && !state.tasks[id].completed) {
          ...
          console.log(`  âœ… "${state.tasks[id].task}" â€” ${state.tasks[id].assignee_name}`);
          try {
    ...
    if (tasks.length > 0) {
      ...
      for (const task of tasks) {
        ...
        if (!state.tasks[taskId]) {
          ...
          newCount++;
          console.log(`\n[NEW] "${task.task}" â†’ ${task.assignee_name} (due: ${task.due_date || 'none'})`);
    ...
    const friday9amWindow = isFriday() && new Date().getUTCHours() === REMINDER_HOUR_UTC;
    const alreadySentThisWeek = state.lastWeeklySummaryDate === thisWeek;

    if (friday9amWindow && !alreadySentThisWeek) {
      console.log('\nIt\'s Friday 9 AM EST â€” sending weekly summary...');
